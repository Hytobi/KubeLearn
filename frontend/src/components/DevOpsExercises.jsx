...
import { devopsExercises, devopsTools } from '../devopsData';
import { CheckCircle2, Lightbulb, Code, Award } from 'lucide-react';
import { Button } from './ui/button';
import { toast } from '../hooks/use-toast';

const DevOpsExercises = ({ score, onScoreUpdate }) => {
  ...
  const currentExercise = devopsExercises[currentExerciseIndex];
  const currentTool = devopsTools.find(t => t.id === currentExercise.toolId);
  const progress = ((completedExercises.size / devopsExercises.length) * 100).toFixed(0);
  ...
  const validateAnswers = () => {
    ...
    if (allCorrect) {
      ...
      toast({
        ...
        description: "Vérifie tes réponses",
        variant: "destructive",
        duration: 2000,
      });
    }
  ...
  const handleNext = () => {
    if (currentExerciseIndex < devopsExercises.length - 1) {
  ...
  const renderCodeWithBlanks = () => {
    let codeContent = currentExercise.template;
    ...
    currentExercise.blanks.forEach(blank => {
      ...
      const inputHtml = `<input 
        ...
        value="${userValue}"
        placeholder="${blank.placeholder.replace(/___/g, '')}"
        class="${inputClass}"
        style="width: ${Math.max(120, blank.placeholder.length * 7)}px;"
      />`;

      codeContent = codeContent.replace(blank.placeholder, inputHtml);
    });
        ...
        <div className="mb-6">
          ...
          <div className="bg-indigo-50 rounded-lg p-4 mb-4">
            ...
            <p className="text-xs text-indigo-700 mt-2">
              {completedExercises.size} / {devopsExercises.length} exercices complétés
            </p>
          </div>
        </div>

        <div className="space-y-2">
          {devopsExercises.map((exercise, index) => {
            const isCompleted = completedExercises.has(exercise.id);
            const isCurrent = index === currentExerciseIndex;
            const tool = devopsTools.find(t => t.id === exercise.toolId);
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span>{tool.icon}</span>
                      <p className="text-xs font-semibold text-gray-500">{exercise.category}</p>
                    </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg border-2 border-gray-200 mb-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-gray-500">
                  Exercice {currentExerciseIndex + 1} / {devopsExercises.length}
                </p>
                <h2 className="text-xl font-bold text-gray-800">{currentExercise.category}</h2>
              </div>
            </div>
            ...
            <div 
              ...
              style={{ 
                backgroundColor: `${currentTool.color}10`,
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="font-semibold mb-1" style={{ color: currentTool.color }}>
                    Objectif :
                  </p>
                  <p style={{ color: currentTool.color }} className="opacity-90">
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg border-2 border-gray-200 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-800">Code à compléter</h3>
              <Button
            ...
            <div className="bg-gray-900 rounded-lg p-4 overflow-x-auto">
              <pre 
                className="text-sm text-green-400 font-mono whitespace-pre"
          ...
          <div className="flex gap-3 justify-between">
            ...
            <Button
              onClick={handleNext}
              disabled={currentExerciseIndex === devopsExercises.length - 1}
              className="px-6 py-3 font-semibold"
              style={{ backgroundColor: currentTool.color }}
            >
              Suivant →
...
export default DevOpsExercises;
